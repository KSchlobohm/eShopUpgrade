...
namespace eShopLegacyMVC.Controllers
{
    public class CatalogController : Controller
    {
        ...
        {
            // MSMQ is not supported in .NET Core/.NET 5+
            // Consider implementing a message broker like RabbitMQ, Azure Service Bus, or other cloud messaging services
            _log.Info($"Item created: {catalogItem.Name}. MSMQ functionality removed as it's not supported in .NET 8.");

            // No-op for now, to be implemented with a cross-platform alternative
        }
        ...
        {
            item.PictureUri = this.Url.RouteUrl(PicController.GetPicRouteName, new { catalogItemId = item.Id }, Request.Scheme);
        }
    }
}